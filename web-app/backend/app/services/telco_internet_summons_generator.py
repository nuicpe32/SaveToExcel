...
# -*- coding: utf-8 -*-
"""
Service สำหรับสร้างหมายเรียกข้อมูล IP Address และซองหมายเรียก
คัดลอกจากระบบโทรศัพท์ แต่เปลี่ยนจากหมายเลขโทรศัพท์เป็น IP Address
"""
...
class TelcoInternetSummonsGenerator:
    """Generator สำหรับสร้างหมายเรียกข้อมูล IP Address"""
    ...
    def generate_telco_internet_letter_html(self, telco_data: Dict, criminal_case: Dict, freeze_account: bool = False) -> str:
        """
        สร้าง HTML หมายเรียกขอข้อมูล IP Address
        คัดลอกจากระบบโทรศัพท์ แต่เปลี่ยนเป็น IP Address
        
        Args:
            telco_data: ข้อมูล IP Address (from telco_internet_accounts table)
        ...
        ip_address = self._format_value(telco_data.get('ip_address', ''))
        
        # แปลง datetime_used เป็นข้อความไทย
        datetime_used_text = ''
        if telco_data.get('datetime_used'):
            from app.utils.thai_date_utils import format_datetime_to_thai
            datetime_used_text = format_datetime_to_thai(telco_data.get('datetime_used'))
        else:
            datetime_used_text = '-'
        ...
        # กำหนดเรื่อง - ใช้ IP Address
        subject_text = f"ขอให้จัดส่งข้อมูลรายละเอียดของ IP Address {ip_address}"
        
        # สร้าง title ตามที่ต้องการ
        html_title = f"หมายเรียกพยานเอกสาร{document_no} ลงวันที่ {date_thai}"
        
        # สร้าง HTML (คัดลอกจากระบบโทรศัพท์)
    ...
    <div class="page">
        ...
        <div class="content">
            ...
            <div class="paragraph">
                ...
                ดำเนินคดีกับคนร้าย ซึ่งมีการใช้งาน IP Address {ip_address} ที่มีส่วนเกี่ยวข้องในกระทำความผิดอาญา 
            ...
            <div class="document-list">
                <ol>
                    <li>IP Address ดังกล่าว ลงทะเบียนไว้ในชื่อบุคคลใด</li>

                    <li>IP Address ดังกล่าว เปิดใช้ตั้งแต่เมื่อใด สถานที่ให้บริการจุดใด</li>

                    <li>มีการขอให้จัดส่งใบแจ้งค่าบริการไปยังที่อยู่ใด</li>

                    <li>ข้อมูลอื่นๆที่จะเป็นประโยชน์ต่อการสืบสวนสอบสวน</li>
                </ol>
            </div>
...
"""
    ...
    def generate_telco_internet_envelope_html(self, telco_data: Dict, telco_address: Optional[Dict] = None) -> str:
        """
        สร้าง HTML ซองหมายเรียกข้อมูล IP Address (ตามรูปแบบระบบธนาคาร)
        ดึงที่อยู่จาก telco_internet table
        
        Args:
            telco_data: ข้อมูล IP Address
            telco_address: ข้อมูลที่อยู่ผู้ให้บริการ (from telco_internet table)
        ...
        # สร้างที่อยู่เต็ม (ถ้ามีข้อมูลจาก telco_internet table)
        address_lines = []
        if telco_address:
